...
describe('fragment-pattern-math', () => {
  ...
  describe('drift pattern', () => {
    it('returns the center (no shell layout)', () => {
      getPatternWorldPosition(_v, PATTERN_DRIFT, 5, 40, 1.2, -0.5, 3.4, 1, { orbitRadius: 2 });
      expect(_v.x).toBeCloseTo(1.2, 5);
      expect(_v.y).toBeCloseTo(-0.5, 5);
      expect(_v.z).toBeCloseTo(3.4, 5);
    });
  });
  ...
  describe('galaxy pattern', () => {
    it('has stronger vertical spread than the flat ring pattern', () => {
      let minG = Infinity;
      let maxG = -Infinity;
      for (let g = 0; g < 80; g++) {
        getPatternWorldPosition(_v, PATTERN_GALAXY, g, 80, 0, 0, 0, 5, { orbitRadius: 2 });
        minG = Math.min(minG, _v.y);
        maxG = Math.max(maxG, _v.y);
      }
      let minR = Infinity;
      let maxR = -Infinity;
      for (let g = 0; g < 80; g++) {
        getPatternWorldPosition(_v, PATTERN_RING, g, 80, 0, 0, 0, 5, { orbitRadius: 2 });
        minR = Math.min(minR, _v.y);
        maxR = Math.max(maxR, _v.y);
      }
      expect(maxG - minG).toBeGreaterThan((maxR - minR) * 2);
    });
