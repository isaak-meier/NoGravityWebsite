...
describe('three-scene helpers', () => {
  ...
  describe('setupGUI', () => {
    it('inserts lil-gui dom element into the document', () => {
      ...
      const { gui } = setupGUI(fakeMat, planetParams, sphere, 1);
    ...
    it('returns gui and radiusCtrl', () => {
      const fakeMat = new THREE.MeshPhysicalMaterial();
      const sphere = new THREE.Mesh();
      const planetParams = { radius: 1 };
      const result = setupGUI(fakeMat, planetParams, sphere, 1);
      expect(result).toHaveProperty('gui');
      expect(result).toHaveProperty('radiusCtrl');
    });
  });
